...
droneButton.onclick=()=>{if(dronePhase!=='off'&&dronePhase!=='return'){stopDrone();return;}lift=0;inertia=0;state.target=state.distance=0;droneSpeed=12.6;dronePhase='walk';droneLabel();};
...
function frame(now){
  ...
  if(dronePhase==='walk'){
    ...
    droneSpeed+=(Math.min(18,Math.max(4,remaining*3))-droneSpeed)*(1-Math.exp(-dt*8));
    move(droneSpeed*dt);
